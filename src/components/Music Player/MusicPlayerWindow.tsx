...
import musicPlayerIcon from "../../assets/music.webp"
...
const MusicPlayerWindow = ({ id, containerRef, zIndex, bringToFront, defaultPosition }: MusicPlayerProps) => {
  const boxRef = useRef<HTMLDivElement>(null)
  const headerRef = useRef<HTMLDivElement>(null)
  const dispatch = useDispatch();
  const isDragging = useRef(false)
  const dragOffset = useRef({ x: 0, y: 0 })
  const minimizeMusicPlayerWindow = useSelector((state: RootState) => state.window.minimizeMusicPlayerWindow)
  ...
  return (
    <Stack
      ...
      sx={{
        ...
        userSelect: "none",
        zIndex,
        display: minimizeMusicPlayerWindow ? "none" : "block"
      }}
    >
      <Stack
        ref={headerRef}
      ...
      >
        <Header onToggleResize={toggleResize} title={"Music Player"} icon={musicPlayerIcon} />
      </Stack>
